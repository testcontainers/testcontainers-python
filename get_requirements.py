...
def __main__() -> None:
    ...
    if args.token:
        ...
        token = input(
            "We need a GitHub access token to fetch the requirements. Please visit "
    ...
    runs = [run for run in response["workflow_runs"] if run["path"].endswith("requirements.yml")]
    if not runs:
        raise RuntimeError(
            "Could not find a workflow. Has the GitHub Action run completed? If you"
    ...
    for artifact in artifacts:
        ...
        response.raise_for_status()
        with zipfile.ZipFile(io.BytesIO(response.content)) as zip, tempfile.TemporaryDirectory() as tempdir:
            zip.extract("requirements.txt", tempdir)
            shutil.move(pathlib.Path(tempdir) / "requirements.txt", pathlib.Path("requirements") / name)
