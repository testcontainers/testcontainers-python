...
class CosmosDbEmulatorContainer(DockerContainer):
    """
        The azure Cosmos DB emulator is still in preview.
        See: https://docs.microsoft.com/en-us/azure/cosmos-db/linux-emulator
        Due to the added complexity of installing ca certs on the local machine so that the emulators cert is trusted, this
        module only runs with ssl_verify = False and is intended for testing purposes
        """

    AZURE_COSMOS_EMULATOR_PARTITION_COUNT = os.environ.get("AZURE_COSMOS_EMULATOR_PARTITION_COUNT", 3)
    AZURE_COSMOS_EMULATOR_ENABLE_DATA_PERSISTENCE = os.environ.get("AZURE_COSMOS_EMULATOR_ENABLE_DATA_PERSISTENCE",
                                                                   False)
